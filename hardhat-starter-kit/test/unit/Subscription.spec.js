    ...
    : describe("Subscription Consumer Tests", async function () {
          ...
          describe("Contract Functionality", async function () {
              ...
              describe("Check Mock Timing Keepers Works Correctly", async function () {
                  ...
                  it("#performUpkeep and test epochs with payment", async function () {
                      const { Subscription } = await loadFixture(deployContractAndPrice)
                      const startingCount = await Subscription.counter()
                      const checkData = ethers.utils.keccak256(ethers.utils.toUtf8Bytes(""))
                      const interval = await Subscription.interval()
                      let pointTwo = ethers.utils.parseEther("0.2") // 4 periods
                      const response = await Subscription.takePayment(deployer.address, 4, {
                          value: pointTwo,
                      })
                      await time.increase(interval.toNumber() + 1)
                  ...
                  it("#performUpkeep and test epochs with payment and return false after payment expires", async function () {
                      const { Subscription } = await loadFixture(deployContractAndPrice)
                      const startingCount = await Subscription.counter()
                      const checkData = ethers.utils.keccak256(ethers.utils.toUtf8Bytes(""))
                      const interval = await Subscription.interval()
                  ...
                  it("It should return true when the Epoch was incremented several times to epoch 5, then payment was rendered to the contract for 5 periods and the epoch period is incremented to 10", async function () {
                      const { Subscription } = await loadFixture(deployContractAndPrice)
                      const startingCount = await Subscription.counter()
                      const checkData = ethers.utils.keccak256(ethers.utils.toUtf8Bytes(""))
                      const interval = await Subscription.interval()
                  ...
                  it("It should return false when the Epoch was incremented several times to epoch 5, then payment was rendered to the contract for 5 periods and the epoch period is incremented to 11", async function () {
                      const { Subscription } = await loadFixture(deployContractAndPrice)
                      const startingCount = await Subscription.counter()
                      const checkData = ethers.utils.keccak256(ethers.utils.toUtf8Bytes(""))
                      const interval = await Subscription.interval()
